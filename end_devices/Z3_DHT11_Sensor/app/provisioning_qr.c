...
int provisioning_qr_build_payload(char *out, size_t out_size)
{
  ...
  // The QR no longer carries the install code (secure provisioning v2): the app
  // sends only eui64 + device_type to the cloud, which resolves the install code
  // from its factory_devices registry. The device's real install code still
  // lives in the MFG_INSTALLATION_CODE token and is used by the Zigbee stack for
  // the install-code-derived join — it is simply never published in the QR.
