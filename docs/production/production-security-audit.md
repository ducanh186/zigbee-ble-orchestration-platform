...
| Backend auth routes | Phase 0 evidence scan found `cloud/app/main.py:57-63` registering health/devices/events/commands/automations/gateways, but no auth router in current main branch. | API endpoints are not proven protected. | Needs Phase 2. |
| Permit join API | `cloud/app/routers/gateways.py:75,108` has open/close permit-join operations. | High-risk operation needs admin-only auth and negative tests. | Needs Phase 2/5. |
