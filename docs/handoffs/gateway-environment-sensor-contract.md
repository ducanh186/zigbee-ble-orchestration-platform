...
- **Verified live (full physical loop on feat/107)** — cloud-format rules synced (`AUTO upsert`) and fired (`AUTO fired ... trigger:threshold_crossed`, `status:executed`, `actions_ok:1`) on real DHT11 reports, **physically toggling the light**: `temperature_c gte 27` → light `004F` ON (`@DATA state:"on"`); `temperature_c lte 30` → light OFF (`@DATA state:"off"`). (One earlier fire logged `unknown_device` because the light auto-paired a few seconds later; once registered, the action executed and the bulb changed state.)
