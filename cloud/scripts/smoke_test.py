...
# Refuse to run against an API that points at a real-EUI device DB. The
# script inserts synthetic light-00124b... rows that pollute the dashboard
# when accidentally aimed at production-shaped state.
...
if not any(t in API_URL for t in _SAFE_API_URL_TOKENS) \
   and not os.environ.get("SB_SMOKE_FORCE"):
    print(
        f"smoke_test: refusing to run against {API_URL}; set "
        "SB_SMOKE_FORCE=1 to override (e.g. for CI runs).",
        file=sys.stderr,
    )
    sys.exit(2)
