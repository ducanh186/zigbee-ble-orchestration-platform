"""Tests for the SCRUM-49 automation event log.

Covers the subscriber that writes ``automation_events`` rows and the
``GET /api/automation-events`` endpoint that mobile reads for the
last-run badge.
"""
...
async def test_automation_executed_event_persisted(db_session_factory):
    ...
    original_run = service._run_async

    def _capture(coro_func):
        tasks.append(asyncio.create_task(coro_func()))

    service._run_async = _capture  # type: ignore[assignment]
    ...
    assert [r.status for r in rows] == ["synced", "executed"]
    assert all(r.automation_id == rule_id for r in rows)
    service._run_async = original_run  # type: ignore[assignment]
