...
## IMPORTANT: Always Read CLAUDE.md First

Before starting ANY task — coding, debugging, reviewing, or answering questions — you MUST read the `CLAUDE.md` file at the repository root **and** any `CLAUDE.md` in the subfolder you are working in. These files exist in every major subfolder (`gateway/`, `cloud/`, `deploy/`, `docs/`, etc.) and contain context specific to that component.

```
CLAUDE.md              ← Read this FIRST (root-level, global context)
gateway/CLAUDE.md      ← Read when working on gateway code
cloud/CLAUDE.md        ← Read when working on cloud code
deploy/CLAUDE.md       ← Read when working on deployment
docs/CLAUDE.md         ← Read when working on documentation
...                    ← Check for CLAUDE.md in any subfolder you touch
```
